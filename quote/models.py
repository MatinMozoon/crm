...
class Quote(models.Model):
    ...
    def total_price(self):
        return self.quote_item.all().annotate(each_item_price=F('quantity') * F('price')) \
            .aggregate(Sum('each_item_price'))['each_item_price__sum']

    def total_price_with_tax(self):
        ip = self.quote_item.all().annotate(each_item_price=F('quantity') * F('price')) \
            .aggregate(Sum('each_item_price'))['each_item_price__sum']
        return ip + ((ip * 9) / 100)
...
class QuoteItem(models.Model):
    ...
    def each_item_price_with_tax(self):
        ip = (self.price * self.quantity) - ((self.price * self.quantity) * self.discount) / 100
        return ip + ((ip * 9) / 100)
